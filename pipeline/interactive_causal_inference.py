...
class InteractiveCausalInferencePipeline(CausalInferencePipeline):
    def __init__(
        ...
        *,
        ...
        cycle_cfg = str(getattr(args, "diagonal_step_cycle", "")).strip()
        self.diagonal_step_cycle: List[int] = [
            int(x) for x in cycle_cfg.split(",") if x.strip()
        ]

    def _build_diagonal_steps(self, step_count: int):
        first_step = int(self.denoising_step_list[0])
        last_step = int(self.denoising_step_list[-1])
        if step_count <= 2:
            return torch.tensor([first_step, last_step], device=self.denoising_step_list.device)
        if step_count == 3:
            return torch.tensor([first_step, 933, last_step], device=self.denoising_step_list.device)
        if step_count >= 4:
            return torch.tensor([first_step, 933, 733, last_step], device=self.denoising_step_list.device)
        ...
        *,
        ...
        for block_index, current_num_frames in enumerate(all_num_frames):
            ...
            if self.use_diagonal_denoising:
                if self.diagonal_step_cycle:
                    cycle_idx = block_index % len(self.diagonal_step_cycle)
                    denoising_step_list = self._build_diagonal_steps(self.diagonal_step_cycle[cycle_idx])
                elif current_start_frame == 0:
                    denoising_step_list = [
                        int(self.denoising_step_list[0]),
                        933,
                        733,
                        int(self.denoising_step_list[-1]),
                    ]
                elif current_start_frame == self.num_frame_per_block:
                    denoising_step_list = [
                        int(self.denoising_step_list[0]),
                        933,
                        int(self.denoising_step_list[-1]),
            ...
            # Refresh policy follows causal_inference:
            # - default: refresh non-last blocks with clean context (t=0)
            # - when not_use_clean_context_kv_update=True: refresh every block with context_noise
            if not self.not_use_clean_context_kv_update:
        ...
        if return_latents:
            ...
            # In-place normalization avoids an additional full-size allocation.
            video = video.mul_(0.5).add_(0.5).clamp_(0, 1)
            return video, output

        # In inference-only mode we can release latent/noise tensors before decode.
        decode_latent = output.to(noise.device)
        del output
        del noise
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

        # Optional chunked decode to keep GPU peak memory bounded on long videos.
        decode_chunk_size = int(getattr(self.args, "decode_chunk_size", 0) or 0)
        if decode_chunk_size > 0 and decode_latent.shape[0] == 1:
            decoded_chunks = []
            total_frames = decode_latent.shape[1]
            ...
            for start in range(0, total_frames, decode_chunk_size):
                ...
                chunk_latent = decode_latent[:, start:end]
                chunk_video = self.vae.decode_to_pixel(chunk_latent, use_cache=True)
                chunk_video = chunk_video.mul_(0.5).add_(0.5).clamp_(0, 1)
                decoded_chunks.append(chunk_video.cpu())
                del chunk_latent
                del chunk_video
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
        ...
        video = self.vae.decode_to_pixel(decode_latent, use_cache=False)
        del decode_latent
        # In-place normalization avoids an additional full-size allocation.
        video = video.mul_(0.5).add_(0.5).clamp_(0, 1)
        return video
