...
# Build a run timestamp once per run (shared across ranks).
if dist.is_initialized():
...
for i, batch_data in tqdm(enumerate(dataloader), disable=(local_rank != 0)):
    ...
    for seed_idx in range(config.num_samples):
        # Build a stable, readable filename with prompt + timestamp + run indices.
        first_prompt_raw = prompts_list[0]
        if isinstance(first_prompt_raw, (list, tuple)):
            first_prompt = str(first_prompt_raw[0])
        else:
            first_prompt = str(first_prompt_raw)
        base_prompt_fragment = first_prompt[:50]
        sanitized_name = ''.join('_' if ch.isspace() else ch for ch in base_prompt_fragment)
        sanitized_name = sanitized_name.replace("/", "_")
        if not sanitized_name:
            sanitized_name = f"prompt_{idx}"

        filename = f"{sanitized_name}_{run_timestamp}_rank{rank}_idx{idx}_seed{seed_idx}_{model_type}.mp4"
        output_path = os.path.join(config.output_folder, filename)
        duplicate_idx = 1
        while os.path.exists(output_path):
            filename = (
                f"{sanitized_name}_{run_timestamp}_rank{rank}_idx{idx}_seed{seed_idx}"
                f"_{model_type}_{duplicate_idx}.mp4"
            )
            output_path = os.path.join(config.output_folder, filename)
            duplicate_idx += 1
