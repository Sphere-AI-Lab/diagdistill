"""Generate long continuous videos (e.g., 5 minutes) without visible cuts.

This script keeps the same text prompt throughout, produces the first chunk
normally, and then repeatedly performs video continuation using the last
segment as context. That way each new chunk begins exactly where the previous
one ended, avoiding abrupt transitions.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Continuous long video inference")
    parser.add_argument("--config_path", required=True)
    parser.add_argument("--num_segments", type=int, default=10, help="Total chunks (>=1)")
    parser.add_argument(
        "--segment_frames",
        type=int,
        default=None,
        help="Latent frames per chunk (default = config.num_output_frames)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="How many frames from the tail to use as continuation context",
    )
...
def prefill_kv_cache(
    ...
    if num_frames % num_frame_per_block != 0:
        ...
        context_latents = torch.cat([context_latents, context_latents[:, -1:].repeat(1, pad, 1, 1, 1)], dim=1)
    ...
    for _ in range(num_blocks):
        frames = context_latents[:, current_start_frame:current_start_frame + num_frame_per_block]
        pipeline.generator(
...
def generate_continuation_latents(
    ...
    if continuation_frames % pipeline.num_frame_per_block != 0:
        ...
        noise = torch.cat([
            noise,
            torch.randn([batch_size, pad, 16, height, width], device=device, dtype=torch.bfloat16),
        ], dim=1)

    output_latents = torch.zeros_like(noise)
    current_start_frame = num_context_frames
...
def main() -> None:
    ...
    if rank == 0:
        print(f"[cont] Device {device}, free VRAM ~{free_vram:.1f} GB, low_memory={low_memory}")
    ...
    assert 0 <= args.prompt_index < len(dataset)
    sample = dataset[args.prompt_index]
    ...
    for seg in range(1, args.num_segments):
        if rank == 0:
            print(f"[cont] Generating continuation segment {seg + 1}/{args.num_segments}")
    ...
    sanitized = sanitize_prompt(prompt, args.prompt_index)
    filename = args.output_name or f"{sanitized}.mp4"
    output_path = os.path.join(run_output_dir, filename)

    write_video(output_path, full_video, fps=args.fps)
    if rank == 0:
        print(f"[cont] Saved continuous video to {output_path}")
