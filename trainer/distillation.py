...
class Trainer:
    ...
    def __init__(self, config):
        ...
        wandb_disabled = self.disable_wandb or (wandb_mode == "disabled")

        # Update self.disable_wandb to reflect the actual state
        self.disable_wandb = wandb_disabled

        if self.is_main_process and not wandb_disabled:
            wandb.login(
        ...
        elif self.is_main_process and wandb_disabled:
            print("Warning: wandb is disabled (WANDB_MODE=disabled or disable_wandb=True)")

        self.output_path = config.logdir
        app_start_time = time.time_ns() / 1_000_000 
        
        # ------------------------------------- One Logger Setup ----------------------------------------------
        # Disabled one_logger due to missing one_logger_utils module
        ...
        # Set one_logger to None to disable it
        self.one_logger = None
        if self.use_one_logger and self.is_main_process:
        ...
        if hasattr(config, 'adapter') and config.adapter is not None:
            ...
            if base_checkpoint_path:
                ...
                if "generator" in base_checkpoint:
                    ...
                    self._safe_load_generator_state_dict(
            ...
            # Initialize optional spatial head before loading LoRA checkpoint so weights can be restored.
            if hasattr(self.model, "initialize_spatial_head"):
                self.model.initialize_spatial_head()
                if self.is_main_process and getattr(self.config, "use_spatial_head", False):
                    print("Spatial head enabled for generator loss (MCM-style)")
                if getattr(self.config, "use_spatial_head", False) and hasattr(self.model.generator, "spatial_head"):
                    base_gen_state = None
                    if "generator" in base_checkpoint:
                        base_gen_state = base_checkpoint["generator"]
                    elif "model" in base_checkpoint:
                        base_gen_state = base_checkpoint["model"]
                    if base_gen_state is not None:
                        spatial_head_state = {
                            k.replace("spatial_head.", "", 1): v
                            for k, v in base_gen_state.items()
                            if k.startswith("spatial_head.")
                        }
                        if len(spatial_head_state) > 0:
                            self.model.generator.spatial_head.load_state_dict(
                                spatial_head_state, strict=True
                            )
                            if self.is_main_process:
                                print("Loaded spatial_head from base checkpoint")
                    if (
                        "target_spatial_head" in base_checkpoint
                        and hasattr(self.model, "target_spatial_head")
                        and self.model.target_spatial_head is not None
                    ):
                        self.model.target_spatial_head.load_state_dict(
                            base_checkpoint["target_spatial_head"], strict=True
                        )
                        if self.is_main_process:
                            print("Loaded target_spatial_head from base checkpoint")
            ...
            if lora_checkpoint_path:
                ...
                if (
                    "spatial_head" in lora_checkpoint
                    and hasattr(self.model, "generator")
                    and hasattr(self.model.generator, "spatial_head")
                ):
                    self.model.generator.spatial_head.load_state_dict(
                        lora_checkpoint["spatial_head"], strict=True
                    )
                    if self.is_main_process:
                        print("Loaded spatial_head from LoRA checkpoint")
                if (
                    "target_spatial_head" in lora_checkpoint
                    and hasattr(self.model, "target_spatial_head")
                    and self.model.target_spatial_head is not None
                ):
                    self.model.target_spatial_head.load_state_dict(
                        lora_checkpoint["target_spatial_head"], strict=True
                    )
                    if self.is_main_process:
                        print("Loaded target_spatial_head from LoRA checkpoint")
            else:
                if self.is_main_process:
                    print("No LoRA checkpoint to load, starting from scratch")

        # Attach optional MCM-style spatial head to generator before FSDP wrapping.
        if hasattr(self.model, "initialize_spatial_head") and not self.is_lora_enabled:
            self.model.initialize_spatial_head()
            if self.is_main_process and getattr(self.config, "use_spatial_head", False):
                print("Spatial head enabled for generator loss (MCM-style)")
        ...
        if not self.is_lora_enabled:
            ...
            if checkpoint_path:
                ...
                # Load target spatial head (if enabled/saved).
                if (
                    "target_spatial_head" in checkpoint
                    and hasattr(self.model, "target_spatial_head")
                    and self.model.target_spatial_head is not None
                ):
                    self.model.target_spatial_head.load_state_dict(
                        checkpoint["target_spatial_head"], strict=True
                    )
                    self.model.target_spatial_head.to(self.device)
                    if self.is_main_process:
                        print("Loaded target_spatial_head from checkpoint")
                elif self.is_main_process and getattr(self.config, "use_spatial_head", False):
                    print("Warning: target_spatial_head not found in checkpoint; using current initialization.")
    ...
    def _safe_load_generator_state_dict(self, state_dict, checkpoint_path: str, key_name: str):
        """Load generator weights with backward compatibility for optional spatial head keys."""
        try:
            self.model.generator.load_state_dict(state_dict, strict=True)
            return
        except RuntimeError as exc:
            if not getattr(self.config, "use_spatial_head", False):
                raise exc
            ...
            if self.is_main_process:
                print(
                    f"Warning: strict load failed for '{key_name}' from {checkpoint_path}; "
                    f"fallback to strict=False (missing={len(missing)}, unexpected={len(unexpected)})."
                )
    ...
    def save(self):
        ...
        if self.is_lora_enabled:
            ...
            if hasattr(self.model, "generator") and hasattr(self.model.generator, "spatial_head"):
                state_dict["spatial_head"] = self.model.generator.spatial_head.state_dict()
            if hasattr(self.model, "target_spatial_head") and self.model.target_spatial_head is not None:
                state_dict["target_spatial_head"] = self.model.target_spatial_head.state_dict()
        else:
            ...
            while True:
                ...
                if self.streaming_training:
                    ...
                    if TRAIN_GENERATOR:
                        ...
                        if hasattr(self.model, "update_target_spatial_head"):
                            self.model.update_target_spatial_head()
