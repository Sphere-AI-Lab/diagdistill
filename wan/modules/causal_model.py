...
def causal_rope_apply(x, grid_sizes, freqs, start_frame=0):
    ...
    for i, (f, h, w) in enumerate(grid_sizes.tolist()):
        seq_len = f * h * w
        if start_frame + f > freqs[0].shape[0]:
            raise RuntimeError(
                f"RoPE temporal index out of range: need frames [{start_frame}, {start_frame + f}), "
                f"but freqs only has {freqs[0].shape[0]} entries. "
                "Increase CausalWanModel.rope_max_frames or reduce num_output_frames."
            )
...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            freqs(Tensor): Rope freqs, shape [rope_max_frames, C / num_heads / 2]
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        Args:
            ...
            rope_max_frames (`int`, *optional*, defaults to 4096):
                Maximum number of latent frames covered by temporal RoPE.
        ...
        self.rope_max_frames = rope_max_frames
        self.local_attn_size = local_attn_size
        self.qk_norm = qk_norm
        self.cross_attn_norm = cross_attn_norm
        self.eps = eps
        ...
        self.freqs = torch.cat([
            rope_params(self.rope_max_frames, d - 4 * (d // 6)),
            rope_params(self.rope_max_frames, 2 * (d // 6)),
            rope_params(self.rope_max_frames, 2 * (d // 6))
        ],
            dim=1)
